...
TaskSchedulingSat.InitTaskList();
int taskCount = TaskSchedulingSat.GetTaskCount();

CpModel model = new CpModel();

IntervalVar[] tasks = new IntervalVar[taskCount];
BoolVar[] taskChoosed = new BoolVar[taskCount];
IntVar[] allEnds = new IntVar[TaskSchedulingSat.GetEndTaskCount()];

int endJobCounter = 0;
foreach (Job j in TaskSchedulingSat.myJobList)
{
    BoolVar[] tmp = new BoolVar[j.AlternativeTasks.Count];
    int i = 0;
    foreach (Task t in j.AlternativeTasks)
    {
        long ti = TaskSchedulingSat.taskIndexes[t.Name];
        ...
        if (!TaskSchedulingSat.tasksToEquipment.ContainsKey(t.Equipment))
            TaskSchedulingSat.tasksToEquipment[t.Equipment] = new List<IntervalVar>();
        TaskSchedulingSat.tasksToEquipment[t.Equipment].Add(tasks[ti]);
    }
    model.AddExactlyOne(tmp);
}

foreach (KeyValuePair<long, List<IntervalVar>> pair in TaskSchedulingSat.tasksToEquipment)
{
...
Console.WriteLine(solver.Response);
...
class TaskSchedulingSat
{
    ...
    public static void InitTaskList()
    {
        List<Task> taskList =
        [
            new Task("Job1Task0a", 15, 0),
            new Task("Job1Task0b", 25, 1),
            new Task("Job1Task0c", 10, 2),
        ];
